...
exports.default = (0, _pundleApi.createResolver)(function () {
  var _ref = _asyncToGenerator(function* (config, givenRequest, fromFile, cached) {
    var _this = this;

    var request = givenRequest;
    var fromDirectory = '';
    var manifest = { rootDirectory: this.config.rootDirectory };
    var extensions = config.extensions || config.knownExtensions;
    var targetManifest = {};

    if (fromFile) {
      fromDirectory = _path2.default.dirname(fromFile);
      Object.assign(manifest, (yield (0, _helpers.getManifest)(fromDirectory, config, cached, this.config)));
    }
    ...
    if ((0, _helpers.isModuleOnly)(request) && request !== 'empty' && _pundleBrowser2.default[request]) {
      return { resolved: _pundleBrowser2.default[request], sourceManifest: manifest, targetManifest: null };
    }
    var resolved = yield (0, _helpers.promisedResolve)(request, {
      ...
      readFile: function readFile(path, callback) {
        _this.config.fileSystem.readFile(path).then(function (result) {
      ...
      isFile: function isFile(path, callback) {
        _this.config.fileSystem.stat(path).then(function (stats) {
